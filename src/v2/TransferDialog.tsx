...
export default function TransferDialog({ ids, sourceLibraryId, onChanged, onClose }: {
  ...
  const [destinationLibraryId, setDestinationLibraryId] = useState<number | null>(initialDest);
    ...
    <div className="fixed inset-0 z-[66] bg-canvas/95 backdrop-blur-sm flex items-center justify-center p-6">
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} className="w-[720px] max-w-full max-h-[86vh] flex flex-col bg-chrome rounded-xl ring-1 ring-line shadow-2xl p-5">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <label className="flex flex-col gap-1"><span className="text-[11px] uppercase tracking-wider text-fg-mute">목적지 라이브러리</span><select value={destinationLibraryId ?? ""} onChange={(e) => setDestinationLibraryId(e.target.value === "" ? null : Number(e.target.value))} className="h-control px-2 rounded bg-raised ring-1 ring-line text-fg">{destinationLibraryId === null && <option value="">목적지를 고르세요</option>}{libs.map((l) => <option key={l.id} value={l.id} disabled={!l.online}>{areaLabel(l.area)} · {l.name}{!l.online ? " (연결 안 됨)" : ""}</option>)}</select></label>
          <div className="flex flex-col gap-1"><span className="text-[11px] uppercase tracking-wider text-fg-mute">동작</span><div className="flex gap-1"><button onClick={() => { modeTouched.current = true; setMode("move"); }} className={`h-control px-3 rounded ring-1 ${mode === "move" ? "bg-accent text-accent-fg ring-accent" : "text-fg-dim ring-line"}`}>이동</button><button onClick={() => { modeTouched.current = true; setMode("copy"); }} className={`h-control px-3 rounded ring-1 ${mode === "copy" ? "bg-accent text-accent-fg ring-accent" : "text-fg-dim ring-line"}`}>복사</button></div></div>
        </div>
