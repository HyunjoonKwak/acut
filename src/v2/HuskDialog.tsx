...
export default function HuskDialog({ libraryId, name, onClose }: { libraryId: number; name: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-canvas/80 backdrop-blur-sm flex items-center justify-center p-6">
      <div className="w-[720px] max-w-full max-h-[85vh] bg-chrome rounded-xl ring-1 ring-line shadow-2xl p-5 flex flex-col">
        <div className="text-[15px] font-semibold text-fg mb-1">「{name}」의 사진 없는 폴더 정리</div>
        <div className="text-[12px] text-fg-mute mb-3">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto scroll-thin rounded-md ring-1 ring-line">
          {list === null ? (
            <div className="p-4 text-fg-mute text-[12.5px]">훑는 중…</div>
          ) : list.length === 0 ? (
            <div className="p-4 text-fg-mute text-[12.5px]">사진 없는 폴더가 없습니다</div>
          ) : (
        ...
        <div className="flex items-center gap-2 mt-4">
          <span className="text-[12.5px] text-fg-dim tabular-nums">
            고른 폴더 <b className="text-fg">{picked.length.toLocaleString()}</b>개 · {files.toLocaleString()}파일 · {fmtBytes(bytes)}
          </span>
          <div className="flex-1" />
          <button onClick={onClose} className="h-control px-3 rounded-md text-fg-dim ring-1 ring-line-strong text-[12.5px]">
            닫기
          </button>
          <button
            onClick={run}
            disabled={busy || picked.length === 0}
            className="h-control px-3 rounded-md bg-drop text-drop-fg font-semibold text-[12.5px] disabled:opacity-40"
          >
