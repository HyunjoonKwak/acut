...
export default function TransferDialog({
  ...
  // 다음 영역에 온라인 라이브러리가 없으면 «선택 안 됨»으로 둔다 — 원본 라이브러리로
  // 폴백하면 그대로 실행했을 때 사진이 자기 루트로 이동한다
  const initialDest = useMemo(() => {
    const area = nextArea(source?.area ?? 3);
    return libs.find((l) => l.online && l.area === area)?.id ?? null;
  }, [libs, source?.area]);
  const [destinationLibraryId, setDestinationLibraryId] = useState<
    number | null
  >(initialDest);
  // 사용자가 이동·복사를 직접 고른 뒤에는 목적지를 바꿔도 되돌리지 않는다
  const modeTouched = useRef(false);
  const [destinationDir, setDestinationDir] = useState("");
  const [newFolder, setNewFolder] = useState(false);
  const [mode, setMode] = useState<Mode>(() =>
    source?.area === 1 && libs.find((l) => l.id === initialDest)?.area === 2
      ? "copy"
      : "move",
  );
  ...
  const publish =
    source?.area === 1 && destination?.area === 2 && mode === "copy";
  const choices = folders.filter(
    (f) => f.library_id === destinationLibraryId && !f.is_library,
  );
  ...
  const inspect = async () => {
    if (destinationLibraryId === null) return;
    setBusy(true);
    setError(null);
    try {
      setPreview(await invoke<Preview>("transfer_preview", { request }));
    } catch (e) {
      setError(String(e));
    } finally {
      setBusy(false);
  ...
  const execute = async () => {
    ...
    try {
      ...
      if (out.failed > 0) {
        setError(
          `${out.completed}장 완료 · ${out.failed}장 실패 — ${out.first_error ?? ""}`,
        );
      } else if (out.completed === 0) {
        const reason =
          out.already_published > 0
  ...
  const conflicts =
    preview?.items.filter((i) => i.conflict !== "none").length ?? 0;
  return (
    <div className="fixed inset-0 z-[66] bg-canvas/95 backdrop-blur-sm flex items-center justify-center p-6">
      <div
        ...
        className="w-[720px] max-w-full max-h-[86vh] flex flex-col bg-chrome rounded-xl ring-1 ring-line shadow-2xl p-5"
      >
          ...
          <label className="flex flex-col gap-1">
            ...
            >
              {destinationLibraryId === null && (
                <option value="">목적지를 고르세요</option>
              )}
              {libs.map((l) => (
                <option key={l.id} value={l.id} disabled={!l.online}>
                  {areaLabel(l.area)} · {l.name}
                  {!l.online ? " (연결 안 됨)" : ""}
                </option>
              ))}
            </select>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`h-control px-3 rounded ring-1 ${mode === "move" ? "bg-accent text-accent-fg ring-accent" : "text-fg-dim ring-line"}`}
              >
                이동
              </button>
              <button
                ...
                className={`h-control px-3 rounded ring-1 ${mode === "copy" ? "bg-accent text-accent-fg ring-accent" : "text-fg-dim ring-line"}`}
              >
                복사
              </button>
            </div>
        ...
        <div className="flex gap-2 mb-3">
          <button
            ...
            className={`h-control px-3 rounded ring-1 ${!newFolder ? "text-accent ring-accent" : "text-fg-dim ring-line"}`}
          >
            기존 폴더
          </button>
          <button
          ...
          {newFolder ? (
            <input
              ...
              onChange={(e) => {
                setDestinationDir(e.target.value);
                setPreview(null);
              }}
              placeholder="예: 2026/가족여행"
              className="h-control flex-1 px-2 rounded bg-raised ring-1 ring-line text-fg"
            />
          ) : (
            <select
              value={destinationDir}
            ...
            >
              ...
              {choices.map((f) => (
                <option
                  key={f.path}
                  value={f.path.split("/").slice(1).join("/")}
                >
        ...
        {publish && (
          <div className="mb-3 px-3 py-2 rounded text-[13px] text-keep ring-1 ring-keep/50 bg-keep/10">
            내사진 → 공용은 복사가 기본입니다. 개인 원본을 유지하고 SHA-256 발행
            원장으로 재실행 중복을 막습니다.
          </div>
        )}
        {preview?.drive_sync_warning && (
          <div className="mb-3 px-3 py-2 rounded text-[12.5px] text-drop ring-1 ring-drop/50 bg-drop/10">
            내사진/공용은 Drive 동기화 폴더입니다. 이동·이름 변경·삭제는 NAS에도
            반영될 수 있습니다.
          </div>
        )}
        <div className="flex items-end gap-2 mb-3">
          ...
          <button
            onClick={inspect}
            disabled={busy || !destination?.online}
            className="h-control px-3 rounded bg-accent text-accent-fg font-semibold disabled:opacity-40"
          >
            충돌 미리보기
          </button>
          {preview && (
            <span className="pb-1 text-[12px] text-fg-mute">
              충돌/기존 발행 {conflicts}건
            </span>
          )}
        </div>
        {preview && (
          <div className="flex-1 min-h-0 overflow-auto rounded ring-1 ring-line bg-canvas mb-3">
            {preview.items.map((i) => (
              <div
                key={i.id}
                className="grid grid-cols-[1fr_1fr_110px] gap-2 px-3 py-2 border-b border-line last:border-0 text-[12px]"
              >
                <span className="truncate text-fg-dim" title={i.source}>
                  {i.source}
                </span>
                <span className="truncate text-fg" title={i.destination}>
                  → {i.destination}
                </span>
                <span
                  className={
                ...
                >
                  {i.conflict === "already_published"
                    ? "이미 발행 · 건너뜀"
                    : i.conflict === "source_missing"
                      ? "원본 없음"
                      : i.conflict === "source_unreadable"
                        ? "원본을 읽을 수 없음"
                        : i.action === "rename"
                          ? "새 이름"
                          : i.action === "skip"
