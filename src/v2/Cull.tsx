...
type Summary = { kind: number; groups: number; photos: number; reclaimable: number };
...
export default function Cull({
  ...
  const loadGroups = useCallback(async (k: number) => {
    const g = await invoke<Group[]>("cull_groups", { kind: k, limit: 200, offset: 0 });
    setGroups(g);
    setIdx(0);
  }, []);
  ...
  useEffect(() => {
    ...
    listen("cull-burst", () => setBusy("같은 순간 완료 — 중복 확인 중")).then((f) =>
      un.push(f),
    );
  ...
  const pick = useCallback(
    async (fileId: number) => {
      ...
      setMembers((m) => m.map((x) => ({ ...x, is_best: x.file_id === fileId })));
    },
    [groups, idx],
  );
  ...
  return (
    <div className="fixed inset-0 bg-[#15191A] text-[#EAEFEF] flex flex-col z-50">
      ...
      <div className="h-12 shrink-0 flex items-center gap-3 px-4 bg-[#1C2123] border-b border-[#242C2E]">
        ...
        {KINDS.map((k) => {
          ...
          return (
            ...
            >
              {k.label}{" "}
              <span className="tabular-nums text-[#6D7B7E]">{s?.groups ?? 0}</span>
            </button>
          );
        })}
        <button
        ...
        {busy && <span className="text-[#F0B429] text-[12px] tabular-nums">{busy}</span>}
      ...
      <div className="h-9 shrink-0 flex items-center gap-3 px-4 bg-[#1B2123] border-b border-[#242C2E] text-[12.5px]">
        ...
        <div className="w-56 h-1.5 rounded bg-[#282F31] overflow-hidden">
          <i
            ...
            style={{
              width: groups.length ? `${((idx + 1) / groups.length) * 100}%` : "0%",
            }}
          />
        </div>
        {cur && (
          <span className="text-[#6D7B7E]">
            {cur.reason} · {cur.member_count}장 · 확보 {fmtBytes(cur.size_bytes)}
          </span>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {!cur && (
          <div className="h-full flex items-center justify-center text-[#6D7B7E]">
            {busy ? busy : "정리할 그룹이 없습니다 — 「다시 찾기」를 눌러보세요"}
          </div>
        )}
        {cur && (
          <div
              ...
              return (
                <button key={m.file_id} onClick={() => pick(m.file_id)} className="text-left">
                  ...
                  <div className="flex justify-between items-baseline mt-1.5 gap-2">
                    <span className="text-[11.5px] text-[#A3B2B4] truncate">{m.name}</span>
      ...
      <div className="h-14 shrink-0 flex items-center gap-2 px-4 bg-[#1C2123] border-t border-[#242C2E]">
        ...
        >
          이대로 확정
          <span className="text-[10.5px] bg-black/20 px-1.5 py-0.5 rounded font-mono">Space</span>
        </button>
        <button
          onClick={skip}
          disabled={!cur}
