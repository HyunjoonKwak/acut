...
const FILES = ["package.json", "package-lock.json", "src-tauri/tauri.conf.json", "src-tauri/Cargo.toml"];
...
function runInSandbox(breakAt, bundleNotice = false) {
  ...
  try {
    ...
    for (const f of [...FILES, "src-tauri/Cargo.lock", "NOTICE"]) {
    ...
    mkdirSync(fakeBin);
    for (const cmd of ["npm", "cargo", "npx"]) {
