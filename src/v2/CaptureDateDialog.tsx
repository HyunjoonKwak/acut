...
export default function CaptureDateDialog({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(found);
      setChecked(
        new Set(found.filter((r) => r.auto_selected).map((r) => r.id)),
      );
    } catch (e) {
      setError(String(e));
    } finally {
  ...
  const run = async (manual: boolean) => {
    ...
    const changes = chosen
      .map((r) => ({
        id: r.id,
        takenAt: manual ? manualTs : r.proposed_at,
        manual,
      }))
      .filter((r): r is { id: number; takenAt: number; manual: boolean } =>
        Number.isFinite(r.takenAt),
      );
    ...
    try {
      ...
      if (out.failed > 0) {
        setError(
          `${out.corrected}장 교정 · ${out.failed}장 실패 — ${out.first_error ?? ""}`,
        );
  ...
  return (
    <div className="fixed inset-0 z-[65] bg-canvas/95 backdrop-blur-sm flex items-center justify-center p-6">
      <div
        ...
        className="w-[920px] max-w-full max-h-[86vh] flex flex-col bg-chrome rounded-xl ring-1 ring-line shadow-2xl p-5"
      >
        ...
        <div className="flex-1 min-h-0 overflow-auto rounded ring-1 ring-line bg-canvas">
          {busy && rows.length === 0 ? (
            <div className="p-5 text-fg-mute">감사하는 중…</div>
          ) : rows.length === 0 ? (
            <div className="p-5 text-fg-mute">감사할 사진이 없습니다.</div>
          ) : (
            rows.map((r) => (
              <label
                key={r.id}
                className="grid grid-cols-[28px_minmax(140px,1fr)_150px_150px_minmax(230px,1.4fr)] gap-2 items-start px-2 py-2 border-b border-line last:border-0 text-[12px]"
              >
                <input
                  type="checkbox"
                ...
                <span className="min-w-0">
                  <span className="block truncate text-fg" title={r.path}>
                    {r.name}
                  </span>
                  <span className="block truncate text-fg-mute" title={r.path}>
                    {r.path}
                  </span>
                </span>
                <span className="text-fg-dim tabular-nums">
                  {new Date(r.current_at * 1000).toLocaleString()}
                  <small className="block text-fg-mute">
                    출처 {r.current_source}
                  </small>
                </span>
                ...
                <span className="text-fg-dim">
                  ...
                  <small className="block text-fg-mute">{r.write_scope}</small>
                  {r.existing_exif && (
                    <small className="block text-keep">
                      유효한 파일 내부 촬영일 — 자동 덮어쓰기 안 함
                    </small>
                  )}
                </span>
              </label>
        ...
        {error && (
          <div role="alert" className="mt-3 text-[13px] text-drop">
            {error}
          </div>
        )}
        <div className="mt-4 flex flex-wrap items-end gap-2">
          <button
            onClick={() => run(false)}
            disabled={busy || chosen.every((r) => !r.auto_selected)}
            className="h-control px-3 rounded-lg bg-accent text-accent-fg font-semibold disabled:opacity-40"
          >
            선택한 자동 후보 교정
          </button>
          ...
          <button
            ...
            disabled={
              busy ||
              chosen.length === 0 ||
              !Number.isFinite(new Date(manualAt).getTime())
            }
            className="h-control px-3 rounded-lg text-fg ring-1 ring-line-strong disabled:opacity-40"
          >
            수동 일괄 교정
          </button>
          ...
          <button
            onClick={onClose}
            disabled={applying}
            className="h-control px-3 rounded-lg text-fg-dim ring-1 ring-line-strong disabled:opacity-40"
          >
            닫기
          </button>
        </div>
        <p className="mt-2 text-[11.5px] text-fg-mute">
          JPEG는 EXIF DateTimeOriginal·DateTimeDigitized·TIFF DateTime과 mtime을
          기록합니다. HEIC·RAW·PNG·영상은 파일 내부 메타데이터를 바꾸지 않고
          mtime과 Photo Desk 보정값만 기록합니다. 모든 성공 항목은 배치 단위로
          되돌릴 수 있습니다.
        </p>
      </div>
    </div>
