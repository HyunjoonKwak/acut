 ...
 */
export default function HuskDialog({
  libraryId,
  name,
  onClose,
}: {
  ...
  const keepish = (h: Husk) =>
    h.kinds.some(([k]) =>
      ["psd", "ai", "zip", "pct", "lrcat", "xmp"].includes(k),
    );
  useEffect(() => {
    let live = true;
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      const [n, err] = await invoke<[number, string | null]>("husk_trash", {
        libraryId,
        rels: picked.map((h) => h.rel),
      });
      toast(
        err
          ? `${n}개 폴더 옮김 · 일부 실패 — ${err}`
          : `${n.toLocaleString()}개 폴더를 휴지통(_폴더)으로 옮겼습니다 — «영구히 비우기»에서 같이 사라집니다`,
        err ? "drop" : "ok",
      );
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-canvas/80 backdrop-blur-sm flex items-center justify-center p-6">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`「${name}」의 사진 없는 폴더 정리`}
        className="w-[720px] max-w-full max-h-[85vh] bg-chrome rounded-xl ring-1 ring-line shadow-2xl p-5 flex flex-col"
      >
        <div className="text-[16px] font-semibold text-fg mb-1">
          「{name}」의 사진 없는 폴더 정리
        </div>
        <div className="text-[13px] text-fg-mute mb-3">
          사진을 다 치운 뒤 카메라 메모(txt)·썸네일(thm)·zip 같은 파일만 남은
          폴더들입니다. 체크된 폴더를 통째로 라이브러리 휴지통(
          <code>.acut/휴지통/_폴더</code>)으로 옮깁니다 — Finder 로 되살릴 수
          있고, «영구히 비우기»에서 같이 사라집니다. 편집 파일(psd·ai·zip 등)이
          든 폴더는 체크를 풀어 두었습니다.
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto scroll-thin rounded-md ring-1 ring-line">
          {list === null ? (
            ...
            <div className="p-4 text-fg-mute text-[13.5px]">
              사진 없는 폴더가 없습니다
            </div>
          ) : (
              ...
              <tbody>
                {list.map((h) => {
                  ...
                  return (
                    <tr
                      key={h.rel}
                      className={`border-t border-line ${on ? "" : "opacity-50"}`}
                    >
                      ...
                      <td
                        className="py-1 pr-3 truncate max-w-[360px]"
                        title={h.rel}
                      >
                        {h.rel}
                      </td>
                      <td className="py-1 pr-3 text-right">
                        {h.files.toLocaleString()}
                      </td>
                      <td className="py-1 pr-3 text-right text-fg-dim">
                        {fmtBytes(h.bytes)}
                      </td>
                      <td
                        className={`py-1 pr-3 ${keepish(h) ? "text-keep" : "text-fg-mute"}`}
                      >
        ...
        <div className="flex items-center gap-2 mt-4">
          <span className="text-[13.5px] text-fg-dim tabular-nums">
            고른 폴더{" "}
            <b className="text-fg">{picked.length.toLocaleString()}</b>개 ·{" "}
            {files.toLocaleString()}파일 · {fmtBytes(bytes)}
          </span>
          <div className="flex-1" />
          <button
            onClick={onClose}
            className="h-control px-3 rounded-md text-fg-dim ring-1 ring-line-strong text-[13.5px]"
          >
            닫기
          </button>
          <button
          ...
          >
            {busy
              ? "옮기는 중…"
              : `고른 ${picked.length.toLocaleString()}개 폴더 휴지통으로`}
          </button>
        </div>
      </div>
    </div>
