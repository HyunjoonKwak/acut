...
describe("정리 부분 실패", () => {
  beforeEach(() => {
    ...
    useData.setState({
      libs: [
        { id: 1, name: "작업대", area: 0, online: true, file_count: 3, volume_uuid: "boot", volume_name: "Macintosh HD", rel_path: "Pictures", dir: "/Pictures" },
      ],
    });
  });
  ...
  it("실패한 사진만 창 안에 남겨 다시 시도한다", async () => {
    ...
    vi.mocked(invoke).mockImplementation(async (command) => {
      ...
      if (command === "organize_move") {
        moves += 1;
        return moves === 1
          ? { batch_id: 1, moved: 2, copied: 0, failed: 1, already_published: 0, bytes: 20, first_error: "한 장 실패", failed_ids: [3], mode: "move" }
          : { batch_id: 2, moved: 1, copied: 0, failed: 0, already_published: 0, bytes: 10, first_error: null, failed_ids: [], mode: "move" };
      }
      return null;
    });
    const onDone = vi.fn();
    const onClose = vi.fn();
    render(<Organize ids={[1, 2, 3]} libraryId={1} onDone={onDone} onClose={onClose} />);
