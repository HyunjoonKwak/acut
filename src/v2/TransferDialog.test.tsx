...
describe("임의 이동·복사와 공용 발행", () => {
  ...
  it("내사진→공용은 복사가 기본이고 미리보기 전에는 실행하지 않는다", async () => {
    ...
    const previewCall = vi
      .mocked(invoke)
      .mock.calls.find(([cmd]) => cmd === "transfer_preview");
    expect(previewCall?.[1]).toMatchObject({
      request: { mode: "copy", publish: true, destinationLibraryId: 2 },
    });
    await waitFor(() =>
      expect(
        vi
          .mocked(invoke)
          .mock.calls.filter(([cmd]) => cmd === "transfer_execute"),
      ).toHaveLength(0),
    );
  });
  ...
  it("전부 건너뛴 실행을 성공처럼 닫지 않고 이유를 보여 준다", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "복사 실행" }),
    );
