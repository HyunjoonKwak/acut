...
test("같은 폴더만 거절 — 품는 관계는 바깥에서 안쪽을 빼고 비교하니 된다", () => {
  assert.equal(
    overlaps(hit("v", "통합전후보"), hit("v", "통합전후보/후보1번")),
    false,
  );
  assert.equal(
    overlaps(hit("v", "공용/2004/주원이사진"), hit("v", "공용/2004")),
    false,
  );
...
test("이름 앞만 같은 것은 겹치지 않는다 — «후보1» 과 «후보10»", () => {
  ...
  assert.equal(
    overlaps(hit("v", "통합전후보/후보1번"), hit("v", "통합전후보/후보2번")),
    false,
  );
});
...
test("일부만 표시됐거나 지울 수 없는 짝이면 처리되지 않았다", () => {
  ...
  assert.equal(
    doneSide(pair({ same: false, b_in_a: false, a_in_b: false, flagged_b: 3 })),
    null,
  );
