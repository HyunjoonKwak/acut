...
export default function FolderSets({ onChanged }: { onChanged: () => void }) {
  ...
  const unmark = useCallback(
    async (targets: FolderSet[]) => {
      ...
      const ok = await ask({
        ...
        lines: [
          "이 묶음 폴더들의 남김·제외 표시를 미판정으로 돌리고, 닫았던 무리는 개별 비교에 다시 나옵니다",
          "파일은 그대로입니다",
        ],
        confirmLabel: "표시 지우기",
      });
  ...
  const applyPicked = useCallback(async () => {
    ...
    const keepAt = (s: FolderSet) =>
      Math.min(keep[setKey(s)] ?? 0, s.folders.length - 1);
    // 지워질 쪽에 NAS 폴더가 있는 묶음 — 사람이 봐야 한다
    const risky = todo.filter((s) =>
      s.folders.some((f, j) => j !== keepAt(s) && settled(f)),
    );
    const bytes = todo.reduce(
      (a, s) => a + s.bytes * (s.folders.length - 1),
      0,
    );
    const files = todo.reduce(
      (a, s) => a + s.files * (s.folders.length - 1),
      0,
    );
    const ok = await ask({
  ...
  const applyAllNas = useCallback(async () => {
    ...
    const todo = sets.filter((s) => {
      ...
      return (
        s.pending &&
        settled(s.folders[k]) &&
        s.folders.every((f, j) => j === k || !settled(f))
      );
    });
    ...
    const ok = await ask({
      ...
      lines: [
        ...
        ...(passed > 0
          ? [
              `${passed.toLocaleString()}묶음은 남길 폴더가 NAS 것이 아니라 건너뜁니다`,
            ]
          : []),
        "파일은 아직 옮기지 않습니다",
      ],
  ...
  if (sets === null)
    return (
      <div className="h-full flex items-center justify-center gap-2 text-fg-mute">
        <i className="w-2 h-2 rounded-full bg-keep animate-pulse" /> 폴더를
        비교하는 중…
      </div>
    );
  const pending = sets.filter((s) => s.pending);
  const gain = pending.reduce(
    (a, s) => a + s.bytes * (s.folders.length - 1),
    0,
  );
  const flagged = sets.reduce((a, s) => a + s.flagged, 0);
    ...
    <div className="h-full flex flex-col relative">
      ...
      <div className="h-11 shrink-0 flex items-center gap-3 px-4 border-b border-line text-[13.5px] bar-fixed">
        <span className="text-fg-dim tabular-nums">
          완전히 같은 폴더(하위 포함){" "}
          <b className="text-fg">{sets.length.toLocaleString()}묶음</b>
          {sets.length >= 5000 && (
            <span className="text-drop">
              {" "}
              (5,000묶음까지만 보입니다 — 처리하면 다음 묶음이 올라옵니다)
            </span>
          )}
          {pending.length > 0 && (
            <>
              {" "}
              · 아직 안 한 것 {pending.length.toLocaleString()}묶음 · 하나만
              남기면 <b className="text-keep">{fmtBytes(gain)}</b> 빔
            </>
          )}
        </span>
        <span className="text-fg-mute">
          — 남길 폴더의 ○을 누르면 ● 남김이 되고 나머지는 제외. 건너뛰려면 그냥
          두세요
        </span>
          ...
          <>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPickedSets(
                    e.target.checked ? new Set(pending.map(setKey)) : new Set(),
                  )
                }
              />
              전체
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 mb-1.5 text-[13px] text-fg-mute tabular-nums">
                ...
                <span>
                  {s.folders.length}곳에 같은 폴더 · {s.files.toLocaleString()}
                  장 · 폴더당 {fmtBytes(s.bytes)}
                </span>
                {!s.pending && (
                ...
                {s.folders.map((f, j) => (
                  ...
                  >
                    <span className="flex items-center justify-center w-16 text-[12px]">
                      ...
                      <span
                        className={
                          k === j ? "text-keep font-semibold" : "text-fg-mute"
                        }
                      >
                        {k === j ? "남김" : "제외"}
                      </span>
                    ...
                    >
                      <span
                        className={settled(f) ? "text-keep" : "text-fg-mute"}
                      >
                        {f.library}
                      </span>
