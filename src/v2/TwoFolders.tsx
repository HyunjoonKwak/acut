...
/** 줄의 열쇠 — 폴더 id 조합 */
const rowKey = (r: PairRow) =>
  `${r.a?.folder_id ?? "x"}-${r.b?.folder_id ?? "x"}`;
...
export default function TwoFolders({ onChanged }: { onChanged: () => void }) {
  ...
  const hashNow = useCallback(async () => {
    ...
    try {
      ...
      toast(
        `${n.toLocaleString()}장에 해시를 붙였습니다 — 다시 비교합니다`,
        "ok",
      );
      setTick((t) => t + 1);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    setBusy(true);
    setRows(null);
    invoke<{ rows: PairRow[]; missing: number; unhashed: number }>(
      "cull_compare_folders",
      {
  ...
  const mark = useCallback(
    async (
      ...
      const pairs = targets.filter((r) =>
        mode === "twins"
          ? !!r.a && !!r.b && r.common > 0
          : droppable(r, side) && doneSide(r) === null,
      );
      if (pairs.length === 0) {
        toast(
          `${side === "a" ? "A쪽 사진이 B쪽에" : "B쪽 사진이 A쪽에"} 다 있는 폴더가 없습니다`,
        );
        return;
      }
      ...
      const files = pairs.reduce(
        (s, r) =>
          s +
          (mode === "twins" ? r.common : side === "a" ? r.files_a : r.files_b),
        0,
      );
  ...
  /// 지워도 되는 짝 — B 쪽 / A 쪽
  const todoB = useMemo(
    () => (rows ?? []).filter((r) => droppable(r, "b") && doneSide(r) === null),
    [rows],
  );
  const todoA = useMemo(
    () => (rows ?? []).filter((r) => droppable(r, "a") && doneSide(r) === null),
    [rows],
  );
  /// 이미 제외 표시가 다 붙은 폴더 수 — 쪽별
  const doneB = useMemo(
    () => (rows ?? []).filter((r) => doneSide(r) === "b").length,
    [rows],
  );
  const doneA = useMemo(
    () => (rows ?? []).filter((r) => doneSide(r) === "a").length,
    [rows],
  );
    ...
    () =>
      (rows ?? []).filter(
        (r) =>
          ...
          r.kept_b === 0 &&
          r.common > 0 &&
          r.flagged_b < r.common,
      ),
    [rows],
  );
  const twinCount = useMemo(
    () => twinRows.reduce((s, r) => s + r.common, 0),
    [twinRows],
  );
  /// 폴더 짝 «보기» — 열려 있는 줄
  ...
  const checkLeft = useCallback(async () => {
    if (!b) return;
    try {
      const l = await invoke<Leftovers>("folder_leftovers", {
  ...
  const unmark = useCallback(
    async (targets: PairRow[]) => {
      const ids = [
        ...new Set(targets.flatMap((r) => [...r.a_ids, ...r.b_ids])),
      ];
      ...
      const ok = await ask({
        ...
        lines: [
          "이 폴더들의 남김·제외 표시를 미판정으로 돌리고, 닫았던 무리는 개별 비교에 다시 나옵니다",
          "파일은 그대로입니다",
        ],
        confirmLabel: "표시 지우기",
      });
      if (!ok) return;
      try {
        const [files] = await invoke<[number, number]>(
          "cull_folder_set_unapply",
          { folderIds: ids },
        );
  ...
  const flagged = useMemo(
    () => (rows ?? []).reduce((s, r) => s + r.flagged_a + r.flagged_b, 0),
    [rows],
  );
  ...
  const sweep = useCallback(async () => {
    ...
    const folderIds = [
      ...new Set(rows.flatMap((r) => [...r.a_ids, ...r.b_ids])),
    ];
    const ok = await ask({
    ...
    setSweeping(true);
    try {
      const r = await invoke<Outcome>("trash_apply", {
        libraryId: null,
        folderIds,
      });
    ...
    <div className="h-full flex flex-col relative">
      ...
      <div className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-line text-[13.5px] flex-wrap">
          ...
          >
            <label className="flex items-center gap-1 cursor-pointer">
              <input
                ...
                name="tf-rows"
                checked={onlyBoth}
                onChange={() => setOnlyBoth(true)}
                className="accent-accent"
              />
              양쪽에 있는 폴더만{" "}
              <span className="text-fg-mute tabular-nums">
                ({bothRows.length.toLocaleString()})
              </span>
            </label>
            <label className="flex items-center gap-1 cursor-pointer">
              <input
        ...
        {rows && !busy && (
          <span className="text-fg-dim tabular-nums">
            폴더 {rows.length.toLocaleString()}개 비교함 — 똑같음{" "}
            <b className="text-fg">{same.length.toLocaleString()}</b> · B쪽
            사진이 A쪽에 다 있음{" "}
            <b className="text-fg">
              {rows.filter((r) => r.b_in_a && !r.same).length.toLocaleString()}
            </b>
        ...
        <div className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-line bg-keep/10 text-[13.5px] flex-wrap">
          <span className="text-fg">
            {review.mode === "twins" ? (
              <>
                <b>
                  B쪽 폴더 {reviewRows.length.toLocaleString()}개 — A쪽에도 있는
                  사진만 제외 표시
                </b>
                <span className="text-fg-dim">
                  {" "}
                  — 폴더는 남고, A에 없는 사진도 남습니다. 체크를 풀면 그 폴더는
                  이번에 빠집니다.
                </span>
              </>
            ) : (
              <>
                <b>
                  {review.side === "a" ? "A" : "B"}쪽 제외 표시할 폴더{" "}
                  {reviewRows.length.toLocaleString()}개
                </b>
                <span className="text-fg-dim">
                  {" "}
                  — 아래 목록을 확인하세요. 체크를 풀면 그 폴더는 이번에
                  빠집니다.
                </span>
              </>
            )}
      ...
      {rows && !review && (
        <div className="shrink-0 flex items-center gap-x-4 gap-y-1 px-4 py-1.5 border-b border-line bg-raised/40 text-[13.5px] flex-wrap">
          ...
          <span className="flex items-center gap-2">
            <span
              className={
                todoB.length + todoA.length > 0 ? "text-fg" : "text-fg-mute"
              }
            >
              ② 제외 표시
            </span>
            {marking ? (
              <span className="flex items-center gap-2 text-keep tabular-nums">
                <i className="w-2 h-2 rounded-full bg-keep animate-pulse" />{" "}
                폴더 {marking.total.toLocaleString()}개에 표시 중…
              </span>
            ) : (
              <>
                {todoB.length > 0 && (
                ...
                {flagged > 0 && (
                  <span className="text-fg-dim tabular-nums">
                    제외 표시된 사진{" "}
                    <b className="text-fg">{flagged.toLocaleString()}</b>장
          ...
          <span className="text-fg-faint">→</span>
          <span className="flex items-center gap-2">
            <span className={flagged > 0 ? "text-fg" : "text-fg-mute"}>
              ③ 휴지통으로
            </span>
            {sweeping ? (
              <span className="flex items-center gap-2 text-keep">
      ...
      {left && rows && rows.every((r) => !r.b) && (
        <div className="shrink-0 flex items-center gap-3 px-4 py-1.5 text-[13px] bg-keep/10 border-b border-line flex-wrap">
          <span className="text-fg">
            B 폴더에 <b>사진이 아닌 파일 {left.files.toLocaleString()}개</b>(
            {fmtBytes(left.bytes)})가 남아 있어 Finder 엔 폴더가 남습니다
            <span className="text-fg-dim">
              {" "}
              —{" "}
              {left.kinds
      ...
      {unhashed > 0 && rows && (
        <div className="shrink-0 flex items-center gap-3 px-4 py-1.5 text-[13px] bg-keep/10 border-b border-line flex-wrap">
          <span className="text-fg">
            ...
            <span className="text-fg-dim">
              {" "}
              — «다시 찾기» 뒤에 들어온 사진이라 아직 내용을 견줄 수 없습니다.
              그 사진이 든 폴더는 «똑같음»이 되지 않습니다.
            </span>
          </span>
          <div className="flex-1" />
          <button
      ...
      {missing > 0 && (
        <div className="shrink-0 px-4 py-1.5 text-[13px] text-drop bg-drop/10 border-b border-line">
          디스크에 없는 폴더 {missing.toLocaleString()}개는 뺐습니다 — Finder
          에서 지운 폴더의 기록이 남은 것입니다. 왼쪽 앨범에서 라이브러리의
          ⟳(다시 스캔)을 누르면 정리됩니다.
        </div>
      )}
      <div className="flex-1 min-h-0 overflow-y-auto scroll-thin">
        {rows === null ? (
          <div className="h-full flex flex-col items-center justify-center gap-2 text-fg-mute text-[14px]">
            <div>
              «폴더 고르기…»로 비교할 두 폴더를 Finder 에서 고르세요 — 등록한
              라이브러리 안의 폴더면 됩니다.
            </div>
            <div className="text-fg-faint text-[13px]">
              예: A = T7 › 통합전후보 › 후보1번 › 연도별, B = T7 › 통합전후보 ›
              후보2번
            </div>
          </div>
        ) : rows.length === 0 ? (
          ...
          <div className="h-full flex items-center justify-center text-fg-mute">
            양쪽에 다 있는 폴더가 없습니다 — «전부»를 고르면 한쪽에만 있는
            폴더도 보입니다
          </div>
        ) : (
          <table className="w-full text-[13.5px] tabular-nums">
            <thead className="text-[11.5px] text-fg-mute uppercase tracking-wider sticky top-0 bg-canvas">
              <tr className="text-left">
                ...
                <th className="py-1.5 px-4 font-medium">
                  A · {a && `${a.library} · ${a.path || "/"}`}
                </th>
                <th className="py-1.5 pr-3 font-medium">
                  B · {b && `${b.library} · ${b.path || "/"}`}
                </th>
...
function Cell({
  ...
  const shown =
    sub &&
    f.library_id === sub.library_id &&
    (rootPath === "" ||
      f.folder === rootPath ||
      f.folder.startsWith(rootPath + "/"))
  ...
  return (
    ...
    >
      ...
      {tree && (
        <span
          className="text-fg-mute text-[11.5px] ml-1"
          title="하위 폴더까지 합쳐서 본 줄"
        >
          /…
        </span>
      )}
...
function Picker({
  ...
  const pick = async () => {
    ...
    const defaultPath = from
      ? from.replace(/\/[^/]+$/, "") || "/Volumes"
      : "/Volumes";
    ...
    try {
      const hit = await invoke<FolderHit | null>("folder_by_path", {
        path: picked,
      });
      if (!hit) {
        toast(
          "등록된 라이브러리 안의 폴더가 아닙니다 — 왼쪽 앨범에 있는 폴더를 고르세요",
          "drop",
        );
        return;
      }
      if (hit.file_count === 0) {
        toast(
          "이 폴더 아래에 아직 훑은 사진이 없습니다 — 라이브러리를 먼저 다시 훑으세요",
          "drop",
        );
        return;
      }
      ...
      <button
        ...
        className={`h-7 min-w-[260px] max-w-[420px] px-2.5 rounded-md text-left truncate ring-1 disabled:opacity-40 ${
          value
            ...
            : "bg-raised text-fg-mute ring-line-strong"
        }`}
      >
        {value ? (
          <>
            ...
            <span className="text-fg-mute">
              {" "}
              ({value.file_count.toLocaleString()})
            </span>
          </>
        ) : (
          "폴더 고르기…"
